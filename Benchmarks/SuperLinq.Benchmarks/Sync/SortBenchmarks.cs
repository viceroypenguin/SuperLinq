...
internal static class PersonData
{
	public static Person[] Generate(int count)
	{
		...
		for (var i = 0; i < count; ++i)
		{
			...
			{
				FirstName = s_firstNames[r.Next(0, s_firstNames.Length)],
				MiddleName = s_firstNames[r.Next(0, s_firstNames.Length)],
				LastName = s_surnames[r.Next(0, s_surnames.Length)],
...
public class TopNBenchmarks : TestDataBenchmark
{
	private readonly Consumer _consumer = new();

	public const int NumberOfPeople = 512;
	...
	[Benchmark]
	public void PartialSortByString() => _people.PartialSortBy(TopN, p => p.FirstName).Consume(_consumer);

	[Benchmark]
	public void PartialSortByValueType() => _people.PartialSortBy(TopN, p => p.DOB).Consume(_consumer);

	[Benchmark]
	public void PartialSortByCustomComparer() => _people.PartialSortBy(TopN, p => p.FavouriteColour, FavourColourComparer.Instance).Consume(_consumer);

	[Benchmark]
	[ArgumentsSource(nameof(NoSpecialization))]
	public void RandomSubset(LinqTestData testData)
	{
		var rand = new Random(12345);
		testData.Collection.RandomSubset(TopN, rand).Consume(_consumer);
	}
}

[BenchmarkCategory]
public class ShuffleBenchmark : TestDataBenchmark
{
	private readonly Consumer _consumer = new();

	[Benchmark]
	[ArgumentsSource(nameof(NoSpecialization))]
	public void Shuffle(LinqTestData testData)
	{
		var rand = new Random(12345);
		testData.Collection.Shuffle(rand).Consume(_consumer);
	}
}
