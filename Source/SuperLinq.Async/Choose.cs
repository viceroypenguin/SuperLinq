...
public static partial class AsyncSuperEnumerable
{
	...
	{
		return Choose(source, x => new ValueTask<(bool, TResult)>(chooser(x)));
	}
		...
		Func<T, ValueTask<(bool, TResult)>> chooser)
	{
		...
		static async IAsyncEnumerable<TResult> _(IAsyncEnumerable<T> source, Func<T, ValueTask<(bool, TResult)>> chooser, [EnumeratorCancellation] CancellationToken cancellationToken = default)
		{
			await foreach (var item in source.WithCancellation(cancellationToken).ConfigureAwait(false))
			{
				var (some, value) = await chooser(item).ConfigureAwait(false);
				if (some)
					yield return value;
			}
		}
